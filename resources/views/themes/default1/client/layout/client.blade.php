                    ...
                    <div class="collapse navbar-collapse links" id="navbarSupportedContent">
                        ...
                        <ul class="navbar-nav navbar-menu site-navigate ml-auto">
                            ...
                            <li @yield('kb') class="nav-item dropdown">
                                <a href="{!! url('knowledgebase') !!}" class="dropdown-toggle nav-link" id="navbarDropdown"
                                    role="button" data-toggle="" aria-haspopup="true"
                                    aria-expanded="false">{!! Lang::get('lang.knowledge_base') !!}
                                </a>
                                <ul class="dropdown-menu" role="menu" aria-labelledby="dropdownMenu">
                                    <li><a href="{{ route('category-list') }}"
                                            class="dropdown-item">{!! Lang::get('lang.categories') !!}</a></li>
                                    <li><a href="{{ route('article-list') }}"
                                            class="dropdown-item">{!! Lang::get('lang.articles') !!}</a></li>
                                </ul>
                            </li>

                            <?php $pages = App\Model\kb\Page::where('status', '1')
                                ->where('visibility', '1')
                                ->get();
                            ?>
                            @if (count($pages))
                                <li @yield('pages') class="nav-item dropdown">
                                    <a class="dropdown-toggle nav-link" id="navbarDropdown" role="button"
                                        ...
                                        aria-expanded="false">{!! Lang::get('lang.pages') !!}
                                    </a>
                                    <ul class="dropdown-menu" role="menu" aria-labelledby="dropdownMenu">
                            ...
                            @if (Auth::user())
                                ...
                                <li @yield('profile') class="nav-item dropdown">
                                    <a href="#" class="dropdown-toggle nav-link" id="navbarDropdown"
                                        role="button" data-toggle="" aria-haspopup="true"
                                        aria-expanded="false">{!! Lang::get('lang.my_profile') !!}
                                    </a>
                                    <ul class="dropdown-menu" role="menu" aria-labelledby="dropdownMenu">
                                        <li>
                                            ...
                                            <div class="banner-wrapper user-data text-center clearfix"
                                                ...
                                                <img id="user_avatar"
                                                    src="{{ Auth::user()->profile_pic }}"class="avatar"
                                                    alt="User Image" height="70" width="70" />

                                                <div><strong>Hello</strong></div>

                                                <p class="banner-title ellipsize_first_name h4">
                                                    {{ Auth::user()->first_name . ' ' . Auth::user()->last_name }}</p>

                                                <div class="banner-content" id="dropdown_content">

                                                    <a href="{{ url('auth/logout') }}"
                                                        class="btn btn-custom btn-sm text-white profile_btn">{!! Lang::get('lang.log_out') !!}</a>

                                                    @if (Auth::user())
                                                        @if (Auth::user()->role != 'user')
                                                            <a href="{{ url('dashboard') }}"
                                                                class="btn btn-custom btn-sm text-white profile_btn">{!! Lang::get('lang.dashboard') !!}</a>
                                                        @endif
                                                    @endif
                                                    @if (Auth::user())
                                                        @if (Auth::user()->role == 'user')
                                                            <a href="{{ url('client-profile') }}"
                                                                class="btn btn-custom btn-sm text-white profile_btn">{!! Lang::get('lang.profile') !!}</a>
                                                        @endif
                                                    @endif
                                                </div>
                                            </div>
                            ...
                            @else
                                @if (isset($errors))
                                    ...
                                    if (is_object($errors)) {
                                        ...
                                        ?>><a
                                            href="#" data-toggle="collapse" class="nav-link"
                                            <?php
                                            if (is_object($errors)) {
                                                if ($errors->first('email') || $errors->first('password')) {
                            ...
                            <li class="nav-item dropdown">
                                ...
                                <ul class="dropdown-menu" style="right: -1px !important;left: auto !important;"
                                    role="menu" aria-labelledby="dropdownMenu" id="lang_ul">
                                    @foreach ($langs as $key => $value)
                                        <?php $src = $key . '.png'; ?>
                                        <li><a href="#" id="{{ $key }}" onclick="changeLang(this.id)"
                        ...
                        <div class="form-inline ">
                            <div class="form-group input-group">
                                <input type="text" name="s" class="search-field form-control input-lg"
                                    title="Enter search term" placeholder="{!! Lang::get('lang.have_a_question?_type_your_search_term_here') !!}" required />
                            </div>
                            <span class="input-group-btn">
                                <button type="submit" class="btn btn-custom btn-md"
                                    style="margin-left: 2px;">{!! Lang::get('lang.search') !!}</button>
                            </span>
                        </div>
                    </div>
                    {!! Form::close() !!}
                ...
                <div class="row">
                    <div class="site-info col-md-6">
                        <p class="text-muted">{!! Lang::get('lang.copyright') !!} &copy; {!! date('Y') !!} <a
                                href="{!! $company->website !!}" target="_blank">{!! $company->company_name !!}</a>.
                        ...
                        <ul class="list-inline hidden-print" style="display: flex;float: right;">
                            @foreach ($socials as $social)
                                @if ($social->name == 'facebook')
                                    @if ($social->value)
                                        <li><a href="{!! $social->value !!}" class="btn btn-social btn-facebook"
                                                target="_blank"><i class="fab fa-facebook fa-fw"></i></a></li>
                                    @endif
                                @endif
                                @if ($social->name == 'twitter')
                                    @if ($social->value)
                                        <li><a href="{{ $social->value }}" class="btn btn-social btn-twitter"
                                                target="_blank"><i class="fab fa-twitter fa-fw"></i></a></li>
                                    @endif
                                @endif
                                @if ($social->name == 'google')
                                    @if ($social->value)
                                        <li><a href="{{ $social->value }}" class="btn btn-social btn-google-plus"
                                                target="_blank"><i class="fab fa-google-plus fa-fw"></i></a></li>
                                    @endif
                                @endif
                                ...
                                @if ($social->name == 'vimeo')
                                    @if ($social->value)
                                        <li><a href="{{ $social->value }}" class="btn btn-social btn-vimeo"
                                                target="_blank"><i class="fab fa-vimeo-square fa-fw"></i></a></li>
                                    @endif
                                @endif
                                ...
                                @if ($social->name == 'rss')
                                    @if ($social->value)
                                        <li><a href="{{ $social->value }}" class="btn btn-social btn-rss"
                                                target="_blank"><i class="fas fa-rss fa-fw"></i></a></li>
                                    @endif
                                @endif
                                @if ($social->name == 'skype')
                                    @if ($social->value)
                                        <li><a href="{{ $social->value }}" class="btn btn-social btn-twitter"
                                                target="_blank"><i class="fab fa-skype fa-fw"></i></a></li>
                                    @endif
                                @endif
                                @if ($social->name == 'stumble')
                                    @if ($social->value)
                                        <li><a href="{{ $social->value }}" class="btn btn-social btn-google-plus"
                                                target="_blank"><i class="fab fa-stumbleupon fa-fw"></i></a></li>
                                    @endif
                                @endif
                                @if ($social->name == 'deviantart')
                                    @if ($social->value)
                                        <li><a href="{{ $social->value }}" class="btn btn-social btn-success"
                                                target="_blank"><i class="fab fa-deviantart fa-fw"></i></a></li>
                                    @endif
                                @endif
                            @endforeach
                        </ul>
