...
<head>
    ...
    <title>OpenDesa | HELP DESK</title>
    ...
    <link href="{{ asset('lb-faveo/adminlte3/plugins/overlayScrollbars/overlayScrollbars.min.css') }}"
    ...
    <!--Daterangepicker-->
    <link rel="stylesheet" href="{{ asset('lb-faveo/css/bootstrap-datetimepicker4.7.14.min.css') }}" rel="stylesheet"
        type="text/css" />

    <link href="{{ asset('lb-faveo/plugins/summernote/summernote-bs4.min.css') }}" rel="stylesheet"
        type="text/css" />
...
<body class="skin-yellow sidebar-mini layout-fixed layout-navbar-fixed text-sm">
            ...
            @if ($replacetop == 0)
                ...
                <ul class="navbar-nav">
                    ...
                    <li class="nav-item d-none d-sm-inline-block">
                        ...
                        <a id="dash" @yield('Dashboard') href="{{ URL::route('dashboard') }}"
                            onclick="clickDashboard(event);" class="nav-link">
                            {!! Lang::get('lang.dashboard') !!}
                        </a>
                    </li>

                    <li class="nav-item d-none d-sm-inline-block">
                        <a href="#tab_user" data-toggle="tab" @yield('Users') class="nav-link"
                            onclick="clickUser(event);" id="user_tab">
                            {!! Lang::get('lang.users') !!}
                        </a>
                    </li>

                    <li class="nav-item d-none d-sm-inline-block">
                        <a href="#tab_ticket" data-toggle="tab" @yield('Tickets') class="nav-link"
                            onclick="clickTickets(event);" id="ticket_tab">
                            {!! Lang::get('lang.tickets') !!}
                        </a>
                    </li>

                    <li class="nav-item d-none d-sm-inline-block">
                        <a href="#tab_tools" data-toggle="tab" @yield('Tools') class="nav-link"
                            onclick="clickTools(event);" id="tools_tab">
                            {!! Lang::get('lang.tools') !!}
                        </a>
                    </li>
                    ...
                    @if ($auth_user_role == 'admin')
                        <li class="nav-item d-none d-sm-inline-block">
                            <a href="{{ URL::route('report.index') }}" onclick="clickReport(event);"
                                @yield('Report') class="nav-link">{!! Lang::get('lang.report') !!}</a>
                        </li>
                    @endif
    ...
    as $notification)
                                        ...
                                        @if ($notification->is_read == 1)
                                            <li class="item" class="task">
                                                ...
                                                <div class="product-img">
                                                    ...
                                                    <img src="{{ $notification->users->profile_pic }}"
                                                        alt="Product Image" class="img-size-50 img-circle">
                                                </div>
                            ...
                            <li class="item" style="position: relative;top: -5px;">
                                ...
                                <img src="{{ asset('lb-faveo/media/images/gifloader.gif') }}"
                                    style="display: none;margin-left: 100px;" id="notification-loader"
                                    class="img-size-50">
                            </li>

                            <li class="dropdown-footer"><a class="text-dark"
                                    href="{{ url('notifications-list') }}">View all</a>
                        </ul>
                    </div>
                </li>
                ...
                <li class="nav-item dropdown">
                    ...
                    <div class="dropdown-menu dropdown-menu-sm dropdown-menu-right p-0" style="width: 290px;">
                        ...
                        @foreach ($langs as $key => $value)
                            <?php $src = $key . '.png'; ?>
                            <a href="#" class="dropdown-item" id="{{ $key }}"
                                onclick="changeLang(this.id)"><img
                                    src="{{ asset("lb-faveo/flags/$src") }}">&nbsp;{{ $value[0] }}&nbsp;
                ...
                <li class="nav-item dropdown user-menu">
                    ...
                    <a href="#" class="nav-link dropdown-toggle" data-toggle="dropdown">
                        @if ($auth_user_id)
                            <img src="{{ $auth_user_profile_pic }}"class="user-image img-circle elevation-2"
                                alt="User Image" />
                    ...
                    <ul class="dropdown-menu dropdown-menu-right">
                        ...
                        <li class="user-header bg-secondary" style="background-color:#343F44;">
                            ...
                            <img src="{{ $auth_user_profile_pic }}" class="img-circle elevation-2"
                                alt="User Image" />
                        ...
                        <li class="user-footer">
                            ...
                            <a href="{{ URL::route('profile') }}"
                                class="btn btn-primary btn-flat">{!! Lang::get('lang.profile') !!}</a>

                            <a href="{{ url('auth/logout') }}"
                                class="btn btn-danger btn-flat float-right">{!! Lang::get('lang.sign_out') !!}</a>
                        </li>
                    </ul>
                </li>
            </ul>
                    ...
                    @else
                        <div class="image">
                            ...
                            <img id="sidebar-profile-img" src="{{ $auth_user_profile_pic }}" alt="User Image"
                                width="auto" height="auto" class="img-circle elevation-2"
                ...
                <nav class="mt-2">
                    ...
                    <ul class="nav nav-pills nav-sidebar flex-column nav-child-indent" data-widget="treeview"
                        ...
                        @if ($replaceside == 0)
                            ...
                            <li class="nav-item">
                                <a href="{{ url('/tickets?show=mytickets') }}" id="load-myticket" @yield('myticket')
                                    class="nav-link">
                            ...
                            <li class="nav-item">
                                <a href="{{ url('/tickets?assigned[]=0') }}" id="load-unassigned"
                                    @yield('unassigned') class="nav-link">
                            ...
                            @foreach ($department as $name => $dept)
                                <li class="nav-item">
                                    ...
                                    <a href="#" @if ($dept2 === $name) @yield('ticket-bar') @endif
                                        class="nav-link">
                                            ...
                                            <ul class="nav nav-treeview">
                                                ...
                                                <li class="nav-item">
                                                    <a href="{!! url('tickets?departments=' . $name . '&status=' . $dept->get($status)->status) !!}"
                                                        @if ($status2 == $dept->get($status)->status && $dept2 === $name) @yield('inbox') @endif
                                                        class="nav-link">
                                                        <i class="far fa-circle nav-icon"></i>
                                                        <p>{!! $dept->get($status)->status !!}</p>
                                                        <small
                                                            class="right badge badge-success">{{ $dept->get($status)->count }}</small>
                                                    </a>
                                                </li>
                                            </ul>
                                        @endif
                    ...
                    <div @yield('user') class="tab-pane" id="tab_user">
                        ...
                        <nav class="navbar navbar-expand navbar-dark navbar-lightblue">
                            ...
                            <ul class="navbar-nav">
                                ...
                                <li class="nav-item d-none d-sm-inline-block">
                                    <a href="{{ url('user') }}" @yield('user-directory')
                                        class="nav-link">{!! Lang::get('lang.user_directory') !!}</a>
                                </li>

                                <li class="nav-item d-none d-sm-inline-block">
                                    <a href="{{ url('organizations') }}" @yield('organizations')
                                        class="nav-link">{!! Lang::get('lang.organizations') !!}</a>
                                </li>
                            </ul>
                        </nav>
                    </div>
                    ...
                    <div @yield('ticket') class="tab-pane" id="tab_ticket">
                        ...
                        <nav class="navbar navbar-expand navbar-dark navbar-lightblue">
                            ...
                            <ul class="navbar-nav">
                                ...
                                <li class="nav-item d-none d-sm-inline-block">
                                    <a href="{{ url('/tickets?last-response-by[]=Client') }}" @yield('open')
                                        class="nav-link" id="load-open">{!! Lang::get('lang.not-answered') !!}</a>
                                </li>

                                <li class="nav-item d-none d-sm-inline-block">
                                    <a href="{{ url('/tickets?last-response-by[]=Agent') }}" @yield('answered')
                                        class="nav-link" id="load-answered">{!! Lang::get('lang.answered') !!}</a>
                                </li>

                                <li class="nav-item d-none d-sm-inline-block">
                                    <a href="{{ url('/tickets?assigned[]=1') }}" @yield('assigned')
                                        class="nav-link" id="load-assigned">{!! Lang::get('lang.assigned') !!}</a>
                                </li>

                                <li class="nav-item d-none d-sm-inline-block">
                                    <a href="{{ url('/tickets?show=closed') }}" @yield('closed')
                                        class="nav-link">{!! Lang::get('lang.closed') !!}</a>
                                </li>

                                <?php if ($group->can_create_ticket == 1) { ?>
                                <li class="nav-item d-none d-sm-inline-block">
                                    <a href="{{ url('/newticket') }}" @yield('newticket')
                                        class="nav-link">{!! Lang::get('lang.create_ticket') !!}</a>
                                </li>
                                <?php } ?>
                            </ul>
                        </nav>
                    ...
                    <div @yield('tool') class="tab-pane" id="tab_tools">
                        ...
                        <nav class="navbar navbar-expand navbar-dark navbar-lightblue">
                            ...
                            <ul class="navbar-nav">
                                ...
                                <li class="nav-item d-none d-sm-inline-block">
                                    <a href="{{ url('/canned/list') }}" @yield('tools')
                                        class="nav-link">{!! Lang::get('lang.canned_response') !!}</a>
                                </li>

                                <li class="nav-item d-none d-sm-inline-block">
                                    <a href="{{ url('/comment') }}" @yield('kb')
                                        class="nav-link">{!! Lang::get('lang.knowledge_base') !!}</a>
                                </li>
                            </ul>
                        </nav>
                    </div>
        ...
        <footer class="main-footer">
            ...
            <span style="font-weight: 500">{!! Lang::get('lang.copyright') !!} &copy; {!! date('Y') !!} <a
                    href="{!! $company->website !!}" target="_blank">{!! $company->company_name !!}</a>.</span>
    ...
    <!-- Slimscroll -->
    <script src="{{ asset('lb-faveo/adminlte3/plugins/overlayScrollbars/overlayScrollbars.min.js') }}"
