...
<head>
    ...
    <!--Daterangepicker-->
    <link rel="stylesheet" href="{{ asset('lb-faveo/plugins/daterangepicker/daterangepicker.css') }}"
        rel="stylesheet" type="text/css" />
    <!--calendar -->
    <!-- fullCalendar 2.2.5-->
    <link href="{{ asset('lb-faveo/plugins/fullcalendar/fullcalendar.min.css') }}" rel="stylesheet"
        type="text/css" />
    <!--[if lt IE 9]>
...
<body class="skin-blue fixed">
    <div class="wrapper">
        <header class="main-header">
            ...
            <nav class="navbar navbar-static-top">
                ...
                <div class="collapse navbar-collapse" id="navbar-collapse">
                    ...
                    @endif
                    <?php $noti = \App\Model\helpdesk\Notification\UserNotification::where('user_id', '=', Auth::user()->id)
                        ->where('is_read', '0')
                        ->get(); ?>
                                        ...
                                        @if ($notification->type == 'registration')
                                            @if ($notification->is_read == 1)
                                                <li class="task" style="list-style: none; margin-left: -30px;">
                                                    <span>&nbsp<img src="{{ $user->profile_pic }}"
                ...
                <ul id="side-bar" class="sidebar-menu">
                    @if ($replaceside == 0)
                        ...
                        if (Auth::user()->role == 'admin') {
                            ...
                            $myticket = App\Model\helpdesk\Ticket\Tickets::where('assigned_to', Auth::user()->id)
                                ->where('status', '1')
                                ->get();
                            $unassigned = App\Model\helpdesk\Ticket\Tickets::where('assigned_to', '=', null)
                                ->where('status', '=', '1')
                                ->get();
                            $tickets = App\Model\helpdesk\Ticket\Tickets::whereIn('status', [1, 7])->get();
                            $followup_ticket = App\Model\helpdesk\Ticket\Tickets::where('status', '1')
                                ->where('follow_up', '1')
                                ->get();
                            ...
                            $unassigned = App\Model\helpdesk\Ticket\Tickets::where('assigned_to', '=', null)
                                ->where('status', '=', '1')
                                ->where('dept_id', '=', Auth::user()->primary_dpt)
                                ->get();
                            $tickets = App\Model\helpdesk\Ticket\Tickets::whereIn('status', [1, 7])
                                ->where('dept_id', '=', Auth::user()->primary_dpt)
                                ->get();
                            $followup_ticket = App\Model\helpdesk\Ticket\Tickets::where('status', '1')
                                ->where('follow_up', '1')
                            ...
                            $deleted = App\Model\helpdesk\Ticket\Tickets::where('status', '5')
                                ->where('dept_id', '=', Auth::user()->primary_dpt)
                                ->get();
                        }
                        if (Auth::user()->role == 'agent') {
                            $dept = App\Model\helpdesk\Agent\Department::where('id', '=', Auth::user()->primary_dpt)->first();
                            $overdues = App\Model\helpdesk\Ticket\Tickets::where('status', '=', 1)
                                ->where('isanswered', '=', 0)
        ...
        $group = App\Model\helpdesk\Agent\Groups::where('id', '=', $agent_group)
                            ...
                            <div class="tabs-pane @yield('user-bar')" id="tabB">
                                <ul class="nav navbar-nav">
                                    <li id="bar" @yield('user')><a
                                            href="{{ url('user') }}">{!! Lang::get('lang.user_directory') !!}</a></li></a></li>
                                    <li id="bar" @yield('organizations')><a
                                            href="{{ url('organizations') }}">{!! Lang::get('lang.organizations') !!}</a></li></a>
                                    </li>
                                </ul>
                            </div>
                            ...
                            <div class="tabs-pane @yield('tools-bar')" id="tabD">
                                <ul class="nav navbar-nav">
                                    ...
                                    <li id="bar" @yield('kb')><a
                                            href="{{ url('/comment') }}">{!! Lang::get('lang.knowledge_base') !!}</a></li>
                                </ul>
                            </div>
                            @if (Auth::user()->role == 'admin')
